...
const SendCard = ()=>{
    ...
    const handleClick = (ev)=>{
        ...
        if (!fullNameIsValid(formData.from.fullName)){
            ...
            const selectedCardImagePath =  card.imagePath[0];
            console.log("selectedCardImagePath",selectedCardImagePath);
            // sanitize email data
            fetch("/sendCard", {
                method: "POST",
                body: JSON.stringify({userId, cardId, formData, selectedCardHtml, selectedCardImagePath}),
                headers: {
            ...
            <Form>
                <Label><b>From</b></Label>
                    <Input type={"text"} 
                        ...
                        highlight={clicked && (fullNameIsValid(formData.from.fullName) ? "default" : "2px solid red")}
                    />

                    <Input type={"email"} 
                        value={user ? user.email : formData.from.email} 
                        placeholder={user ? '' :  "Email"}
                        onChange={user ? ()=>{} : (ev)=>{setFormData({...formData, from: {...formData.from, email: ev.target.value}})}}
                        highlight={clicked && (user ? (emailIsValid(user.email) ? "default" : "2px solid red") : (emailIsValid(formData.from.email) ? "default" : "2px solid red") )}
                    />
                ...
                <Label><b>To</b></Label>
                    <Input type={"email"} 
                        ...
                        highlight={clicked && (emailIsValid(formData.to.email) ? "default" : "2px solid red")}
                    />
